...
LinkLuaModifier("modifier_phantom_fleet_slow","lua/abilities/lysander_phantom_fleet",LUA_MODIFIER_MOTION_NONE)
...
if IsServer() then
	function lysander_phantom_fleet:OnSpellStart()
		...
		local t_damage_bonus = caster:FindTalentValue("special_bonus_lysander_1") or 0
	...
	function lysander_phantom_fleet:OnProjectileHit(hTarget, vLocation)
		if hTarget then
			...
			self:InflictDamage(hTarget,self:GetCaster(),damage,DAMAGE_TYPE_PHYSICAL)
		end
	end
end
