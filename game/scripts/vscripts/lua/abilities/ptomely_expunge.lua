...
LinkLuaModifier("modifier_expunge","lua/abilities/ptomely_expunge",LUA_MODIFIER_MOTION_NONE)
	...
	if IsServer() then
		...
		Timers:CreateTimer(self:GetDuration()-0.1,function()
			...
			for k,v in pairs(enemy_found) do
				self:GetAbility():InflictDamage(v,caster,drain,DAMAGE_TYPE_MAGICAL)
			end
