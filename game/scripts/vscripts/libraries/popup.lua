...
-- e.g. when dealing special damage
function PopupSpecDamage(target, amount)
    PopupNumbers(target, "crit", Vector(128, 128, 255), 2.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

-- e.g. when dealing special damage
function PopupFuryDamage(target, amount)
    PopupNumbers(target, "crit", Vector(255, 50, 50), 0.75, amount, nil, nil)
end
