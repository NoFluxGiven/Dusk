...
================================================================================================================= ]]
function talos_start(keys)	
	keys.caster:EmitSound("DOTA_Item.Mekansm.Activate")
	ParticleManager:CreateParticle("particles/items/talos.vpcf", PATTACH_ABSORIGIN, keys.caster)
	...
	for i, nearby_ally in ipairs(nearby_allied_units) do  --Restore health and play a particle effect for every found ally.
		if not nearby_ally:HasModifier("modifier_item_talos_datadriven_heal_debuff") then
			nearby_ally:Heal(keys.HealAmount, keys.caster)
		end
		
		nearby_ally:EmitSound("DOTA_Item.Mekansm.Target")
		ParticleManager:CreateParticle("particles/items/talos_recipient.vpcf", PATTACH_ABSORIGIN_FOLLOW, nearby_ally)
		
		keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "modifier_item_talos_datadriven_heal_regen", nil)
		keys.ability:ApplyDataDrivenModifier(keys.caster, nearby_ally, "modifier_item_talos_datadriven_heal_debuff", nil)
	end
end